...
export function StoryStatsBanner() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/intelligence/sip-vs-panic/story-stats", { cache: "no-store" });
        const json = (await res.json()) as StoryStats;
    ...
    <div className="mt-4 rounded-2xl border border-white/10 bg-black/25 px-4 py-3">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div className="inline-flex items-center gap-2 text-xs text-white/75">
          ...
          <span className="font-semibold text-white/85">Story stats</span>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-white/80">
          <span>
            <span className="font-semibold text-white">{view.choices.stopPct}%</span> stop
          </span>
          <span className="text-white/35">•</span>
          <span>
            <span className="font-semibold text-white">{view.choices.continuePct}%</span> continue
          </span>
          <span className="text-white/35">•</span>
          <span>
            <span className="font-semibold text-white">{view.choices.pause6Pct + view.choices.pause12Pct}%</span> pause
          </span>
          <span className="text-white/35">•</span>
          <span>
            <span className="font-semibold text-white">Avg cost:</span>{" "}
            <LakhTooltip
              amount={view.avgBehavioralCost}
              decimals={1}
      ...
      <div className="mt-1 text-[11px] text-white/55">Live from real simulator completions (no curated numbers).</div>
    </div>
  );
}
