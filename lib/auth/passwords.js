...
export function isAdminPasswordConfigured() {
  return Boolean(process.env.ADMIN_PASSWORD_HASH || process.env.ADMIN_PASSWORD || CONSTANTS?.AUTH?.ADMIN_PASSWORD_HASH_FALLBACK);
}

export function isFamilyAdminPasswordConfigured() {
  return Boolean(process.env.FAMILY_ADMIN_PASSWORD_HASH || process.env.FAMILY_ADMIN_PASSWORD || CONSTANTS?.AUTH?.FAMILY_ADMIN_PASSWORD_HASH_FALLBACK);
}

export function verifyAdminPassword(input) {
  const pwd = String(input ?? "");
  const hash = process.env.ADMIN_PASSWORD_HASH || CONSTANTS?.AUTH?.ADMIN_PASSWORD_HASH_FALLBACK;
