...
// 🔵 CREATE / UPDATE
export async function upsertLead({ name, email, phone, interest, source }) {
  const sb = supabaseAdmin();
  const e = normalizeEmail(email);
  if (!e) throw new Error("email_required");

  const row = { name: String(name || "").trim() || null, email: e, phone: String(phone || "").trim() || null };
  if (interest) row.interest = String(interest).slice(0, 200);
