...
function getSurchargeRateFY2526(taxableIncome) {
  const ti = Math.max(0, Number(taxableIncome) || 0);
  if (ti > 5_00_00_000) return { rate: 0.37, threshold: 5_00_00_000 };
  if (ti > 2_00_00_000) return { rate: 0.25, threshold: 2_00_00_000 };
  if (ti > 1_00_00_000) return { rate: 0.15, threshold: 1_00_00_000 };
  if (ti > 50_00_000) return { rate: 0.10, threshold: 50_00_000 };
  return { rate: 0, threshold: null };
}

function getPrevSurchargeRateFY2526(threshold) {
  // Rate applicable at the exact threshold (i.e., the lower bracket).
  if (threshold === 5_00_00_000) return 0.25;
  if (threshold === 2_00_00_000) return 0.15;
  if (threshold === 1_00_00_000) return 0.10;
  if (threshold === 50_00_000) return 0;
  return 0;
}

function computeTaxBeforeCessForTaxableIncome({ taxableIncome, regime }) {
  const slabsNew = [
  ...
  const ti = Math.max(0, Number(taxableIncome) || 0);
  const slabRes = computeSlabTaxWithBreakdown(ti, slabs, topRate);
  const baseTaxBeforeRebate = slabRes.tax;
  const taxAfterRebate = applyRebate({ baseTax: baseTaxBeforeRebate, taxableIncome: ti, regime });
  const mr = !isOld
    ? applyNewRegimeMarginalRelief({ taxBeforeCess: taxAfterRebate, taxableIncome: ti })
    : { tax: taxAfterRebate, relief: 0 };
  return { tax: mr.tax, rebateAppliedTax: taxAfterRebate, marginalRelief: mr.relief };
}

function applySurchargeWithMarginalReliefFY2526({ taxBeforeCess, taxableIncome, regime }) {
  const baseTax = Math.max(0, Number(taxBeforeCess) || 0);
  const ti = Math.max(0, Number(taxableIncome) || 0);

  const { rate, threshold } = getSurchargeRateFY2526(ti);
  if (!rate || !threshold) {
    return {
      surchargeRate: 0,
      surcharge: 0,
      surchargeMarginalRelief: 0,
      taxAfterSurcharge: baseTax,
    };
  }
  ...
  // Marginal relief: total tax (before cess) should not exceed
  // (tax at threshold) + (income - threshold).
  const prevRate = getPrevSurchargeRateFY2526(threshold);
  const thresholdTaxRes = computeTaxBeforeCessForTaxableIncome({ taxableIncome: threshold, regime });
  const thresholdTotal = thresholdTaxRes.tax * (1 + prevRate);
  const capTotal = thresholdTotal + (ti - threshold);

  if (rawTotal > capTotal) {
    const cappedSurcharge = Math.max(0, capTotal - baseTax);
    const relief = rawTotal - capTotal;
    return {
      surchargeRate: rate,
      surcharge: cappedSurcharge,
      surchargeMarginalRelief: relief,
      taxAfterSurcharge: baseTax + cappedSurcharge,
    };
  }
...
export function calculateTaxFY2526({
  ...
  const salary = clampNumber(annualSalary, 0, 10_00_00_000);
  ...
  const basic = clampNumber(basicSalary || salary * 0.5, 0, 10_00_00_000);
  ...
  const surchargeRes = applySurchargeWithMarginalReliefFY2526({
    taxBeforeCess: taxAfterRelief,
    taxableIncome,
    regime,
  });
  const taxAfterSurcharge = surchargeRes.taxAfterSurcharge;

  const cess = (taxAfterSurcharge || 0) * 0.04;
  const totalTax = (taxAfterSurcharge || 0) + cess;
  ...
  return {
    ...
    surchargeMarginalRelief: surchargeRes.surchargeMarginalRelief,
    taxAfterSurcharge,
