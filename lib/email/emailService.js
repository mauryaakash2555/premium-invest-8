...
export const EmailService = {
  ...
  async sendWithAttachments({ to, subject, html, attachments = [] }) {
    ...
    const safeAttachments = Array.isArray(attachments)
      ? attachments
          .map((a) => ({
            filename: String(a?.filename || '').trim() || 'attachment.pdf',
            content: a?.content,
  ...
  async sendLeadCapturedAlert({ to, lead, source = null, note = null }) {
    const dashboardUrl = String(process.env.PUBLIC_DASHBOARD_URL || 'https://bmwealth.co.in/admin-secret-akash');
    const name = safeHtml(lead?.name || 'Anonymous');
    const email = safeHtml(lead?.email || '-');
    const phone = safeHtml(lead?.phone || '-');
    const createdAt = lead?.created_at ? new Date(lead.created_at).toLocaleString() : new Date().toLocaleString();
    const src = safeHtml(source || 'unknown');
    const extra = note ? `<p><strong>Note:</strong> ${safeHtml(note)}</p>` : '';
    ...
    const content = `
      ...
      ${extra}
      <p>
        <a class="button" href="${safeHtml(dashboardUrl)}">Open Super Admin Dashboard →</a>
      </p>
      <p style="color:#666;font-size:12px;">BM Wealth Auto-Notification System</p>
    `;
    ...
    return this.sendRaw({
      to,
      subject: `NEW LEAD: ${lead?.name ? String(lead.name).trim() : 'Anonymous'}`,
      html: emailTemplate(content),
    });
  },

  async sendContactFormAlert({ to, contact }) {
    const dashboardUrl = String(process.env.PUBLIC_DASHBOARD_URL || 'https://bmwealth.co.in/admin-secret-akash');
    const name = safeHtml(contact?.name || 'Anonymous');
    const email = safeHtml(contact?.email || '-');
    const phone = safeHtml(contact?.phone || '-');
    const message = safeHtml(contact?.message || '-');
    const page = safeHtml(contact?.page || '/contact');
    const when = safeHtml(new Date().toLocaleString());
    ...
    return this.sendRaw({
      to,
      subject: `CONTACT: ${contact?.name ? String(contact.name).trim() : 'New message'}`,
      html: emailTemplate(content),
    });
  },

  async sendWhatsAppInboundAlert({ to, inbound }) {
    const dashboardUrl = String(process.env.PUBLIC_DASHBOARD_URL || 'https://bmwealth.co.in/admin-secret-akash');
    const phone = safeHtml(inbound?.phone || '-');
    const provider = safeHtml(inbound?.provider || 'unknown');
    const body = safeHtml(inbound?.body || '-');
    const when = safeHtml(new Date().toLocaleString());

    const content = `
      <h2>New WhatsApp Message Received</h2>
      <p><strong>Time:</strong> ${when}</p>
    ...
    return this.sendRaw({
      to,
      subject: `WHATSAPP INBOUND: ${inbound?.phone ? String(inbound.phone).trim() : 'New message'}`,
      html: emailTemplate(content),
    });
  },
};
