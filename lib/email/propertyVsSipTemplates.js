...
export function buildPropertyVsSipFreeSummaryEmail({ lead, inputs, siteUrl, tracking = null }) {
  ...
  const baseUrl = String(siteUrl || process.env.NEXT_PUBLIC_SITE_URL || "https://www.bmwealth.co.in").replace(/\/+$/, "");
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;line-height:1.6;color:#111;max-width:700px">
      ...
      <p style="margin:0">I’ve made your <strong>Property Report</strong>.</p>
      <p style="margin:10px 0 0">This is a calculator, not advice.</p>

      <p style="margin:16px 0 16px">
        <a href="${safeHtml(trackedCtaUrl)}" style="background:${EMAIL_ACCENT_HEX};color:#111;text-decoration:none;padding:12px 18px;border-radius:6px;display:inline-block">
          DOWNLOAD MY REPORT
        </a>
      </p>
