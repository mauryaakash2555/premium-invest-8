...
function stampFooterAllPages(doc, footerText) {
  ...
  const brandLine = "BM WEALTH • bmwealth.co.in • +91 8850977259 • ARN 90008";
  const disclaimerLine = "Education-only. Not investment/tax/legal advice.";
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const lines = [brandLine, text || disclaimerLine, `Page ${i} of ${pageCount}`].filter(Boolean);
    const wrapped = lines
      .map((l) => doc.splitTextToSize(String(l), maxWidth))
      .reduce((acc, cur) => acc.concat(cur), []);
    const clamped = clampLines(wrapped, 3);
