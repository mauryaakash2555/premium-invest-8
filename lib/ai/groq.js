...
export async function callGroqSafe({
  ...
  try {
    const { reply, usage } = await callGroqDetailed({ apiKey, userText, conversationHistory, system, maxTokens, temperature, model });
 ...
 */
export async function callGroq({ apiKey, userText, conversationHistory = [], system = "", maxTokens, temperature, model }) {
  const { reply } = await callGroqDetailed({ apiKey, userText, conversationHistory, system, maxTokens, temperature, model });
  return reply;
}
 ...
 */
export async function callGroqDetailed({
  apiKey,
  userText,
  conversationHistory = [],
  system = "",
  ...
  const body = {
    model: String(model || CONSTANTS.AI_MODELS.GROQ),
    temperature: Number.isFinite(Number(temperature)) ? Number(temperature) : 0.35,
    max_tokens: Number.isFinite(Number(maxTokens)) ? Number(maxTokens) : 260,
    messages,
  };
