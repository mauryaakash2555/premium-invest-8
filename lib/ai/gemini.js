...
export async function callGeminiSafe({
  ...
  try {
    const { reply, usage } = await callGeminiDetailed({ apiKey, userText, conversationHistory, system, maxTokens, temperature, model });
 ...
 */
export async function callGemini({ apiKey, userText, conversationHistory = [], system = "", maxTokens, temperature, model }) {
  const { reply } = await callGeminiDetailed({ apiKey, userText, conversationHistory, system, maxTokens, temperature, model });
  return reply;
}
...
export async function callGeminiDetailed({
  ...
  const useModel = String(model || CONSTANTS.AI_MODELS.GEMINI);
  const url =
  ...
  const body = {
    ...
    generationConfig: {
      ...
      maxOutputTokens: Number.isFinite(Number(maxTokens)) ? Number(maxTokens) : 260,
    },
  };
