...
function normalizeMainSiteUrl(input) {
  const raw = String(input || '').trim() || 'https://bmwealth.co.in';
  let url;
  try {
    url = new URL(raw);
  } catch {
    url = new URL('https://bmwealth.co.in');
  }

  // Force HTTPS.
  url.protocol = 'https:';

  // Force non-www for the main site.
  const h = url.hostname.toLowerCase();
  if (h === 'www.bmwealth.co.in') url.hostname = 'bmwealth.co.in';

  return url.toString().replace(/\/$/, '');
}

const SITE_URL = normalizeMainSiteUrl(process.env.NEXT_PUBLIC_SITE_URL || 'https://bmwealth.co.in');
