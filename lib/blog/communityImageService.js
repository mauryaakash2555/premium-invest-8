...
import { upsertCommunityImageOverride } from './communityImageOverrides';
...
export async function generateImageForPost(post, options = {}) {
  ...
  try {
    ...
    if (searchResult.results && searchResult.results.length > 0) {
      ...
      const url = img?.urls?.default || img?.urls?.highQuality || img?.urls?.raw || null;
      return {
        url,
        source: 'unsplash',
        keywords,
        unsplashId: img.id,
        photographer: img?.photographer?.name || 'Unknown',
        photographerUrl: img?.photographer?.link || null
      };
    }
  } catch (err) {
...
export async function updatePostImage(postId, imageUrl, imageKeywords = []) {
  ...
  try {
    ...
    const supaRes = await upsertCommunityImageOverride({
      postId,
      imageUrl,
      keywords: imageKeywords,
      source: 'manual',
    }).catch((e) => ({ success: false, error: e?.message || 'Supabase override failed' }));

    if (!fileOk && !supaRes?.success) {
      return {
        success: false,
        error: supaRes?.error || 'Failed to persist image update',
        hint: supaRes?.hint || null,
      };
    }
    ...
    return {
      ...
      persisted: {
        file: fileOk,
        supabase: Boolean(supaRes?.success),
        hint: supaRes?.hint || null,
      },
    };
  } catch (err) {
...
export async function rotatePostImage(postId) {
  ...
  try {
    ...
    const supaRes = await upsertCommunityImageOverride({
      ...
      keywords: Array.isArray(post.image_keywords) ? post.image_keywords : [],
      source: 'rotated',
    }).catch((e) => ({ success: false, error: e?.message || 'Supabase override failed' }));
    ...
    if (!fileOk && !supaRes?.success) {
      return {
        success: false,
        error: supaRes?.error || 'Failed to persist rotation',
        hint: supaRes?.hint || null,
      };
    }
  ...
  try {
    ...
    return {
      ...
      images: (result.results || []).map((img) => ({
        id: img.id,
        url: img?.urls?.default || img?.urls?.highQuality || img?.urls?.raw || null,
        thumbnail: img?.urls?.thumbnail || null,
        description: img?.description || 'Unsplash image',
        photographer: img?.photographer?.name || 'Unknown',
        photographerUrl: img?.photographer?.link || null,
      })),
    };
