...
export const MODES = {
  ...
  live_market: {
    ...
    // Spec: Green/Red dynamic (we use a premium green as the base accent)
    accentColor: 'rgba(90, 235, 170, 1)',
    accentColorDim: 'rgba(90, 235, 170, 0.20)',
    glowColor: 'rgba(90, 235, 170, 0.35)',
  },
  market_close: {
    key: 'market_close',
    label: 'Market Close',
    ...
    // Spec: Gold accent
    accentColor: 'rgba(192, 160, 98, 1)',
    accentColorDim: 'rgba(192, 160, 98, 0.22)',
    glowColor: 'rgba(192, 160, 98, 0.33)',
  },
  evening_intel: {
    key: 'evening_intel',
 ...
 * Note: This Date's underlying timestamp is still absolute; treat its
 * getters (getHours/getDate/etc.) as IST-only when created via this helper.
 */
...
/**
 * Get YYYY-MM-DD in IST (safe for cache keys).
 */
export function getISTDateKey(date = null) {
  const ist = date instanceof Date ? date : getISTNow();
