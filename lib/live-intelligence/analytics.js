...
export function getSessionId() {
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      // Some environments (privacy modes / strict settings) can throw on storage access.
      sessionId = null;
    }
  }

  if (!sessionId) {
    sessionId = typeof window === 'undefined' ? `li_ssr_${Date.now()}` : `li_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('beforeunload', () => {
    if (eventQueue.length > 0) {
      // Use sendBeacon for reliable delivery (best-effort)
      try {
