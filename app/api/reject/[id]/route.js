...
export async function POST(req, { params }) {
  ...
  try {
    ...
    const upstream = await fetch(`${BACKEND_ORIGIN}/api/reject/${encodeURIComponent(id)}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
        ...(got ? { 'x-admin-token': got } : {}),
      },
    ...
    if (!upstream.ok) {
      const detail = typeof data === 'object' && data && 'detail' in data ? data.detail : typeof data === 'string' && data ? data : 'Reject failed';
