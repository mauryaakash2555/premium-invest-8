...
export async function POST(req) {
  ...
  try {
    ...
    const upstream = await fetch(`${BACKEND_ORIGIN}/api/newsletter`, {
      method: 'POST',
    ...
    if (!upstream.ok) {
      const detail = typeof data === 'object' && data && 'detail' in data ? data.detail : typeof data === 'string' && data ? data : 'Subscribe failed';
