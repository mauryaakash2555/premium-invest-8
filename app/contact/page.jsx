    ...
    try {
      ...
      const response = await axios.post(`/api/contact`, {
        ...
        validateStatus: function (status) {
          // Let us read JSON error details for non-2xx.
      ...
      // - Treat any 2xx as success unless the API explicitly returns success: false.
      if (response?.status < 200 || response?.status >= 300 || response?.data?.success === false) {
        const detail = response?.data?.detail;
        throw new Error(typeof detail === 'string' && detail.trim() ? detail : 'Form submission failed');
      }
      ...
      setSubmitNote("Message sent. We'll get back to you shortly.");
      ...
      if (shouldShowFallback) {
        setShowWhatsAppFallback(true);
        setSubmitNote("Unable to send right now. Please use WhatsApp below.");
        ...
        setShowWhatsAppFallback(false);
        setSubmitNote("Unable to send. Please check your details and try again.");
      }
    } finally {
            ...
            <form onSubmit={handleSubmit} data-testid="contact-form">
              <div style={{ marginBottom: '24px' }}>
                ...
                <input
                  ...
                  onBlur={(e) => (e.target.style.borderColor = 'color-mix(in oklab, var(--lux-accent) 28%, transparent)')}
                />
              </div>
                ...
                <div
                  style={{
                    ...
                    border: submitNote.startsWith('Message sent')
                      ? '1px solid color-mix(in oklab, var(--lux-accent) 35%, transparent)'
                      : '1px solid rgba(255,255,255,0.10)',
                    background: submitNote.startsWith('Message sent')
                      ? 'rgba(255,255,255,0.04)'
                      : 'rgba(255, 255, 255, 0.03)',
                    color: submitNote.startsWith('Message sent') ? 'var(--lux-accent)' : '#CCCCCC',
                    fontSize: '14px',
                    lineHeight: 1.35,
                  }}
                >
              ...
              {showWhatsAppFallback && (
                <div data-whatsapp-fallback className="mt-6 text-center">
                  ...
                  >
                    <span className="bm-wa-fallback__icon" aria-hidden>
                    ...
                    <span className="text-[15px] font-semibold tracking-wide">WhatsApp Concierge</span>
                  </a>
                  ...
                  <style jsx>{`
                    .bm-wa-fallback {
                      position: relative;
                      overflow: hidden;
                      animation: bmWaPulse 2.2s ease-in-out infinite;
                    ...
                    .bm-wa-fallback::before {
                      ...
                      background: linear-gradient(
                        ...
                        transparent 55%,
                        color-mix(in oklab, var(--color-matte-gold) 10%, transparent) 100%
                      );
                      opacity: 0.75;
                      pointer-events: none;
                    }

                    .bm-wa-fallback:hover {
                      transform: translateY(-1px);
                      color: #000;
                      background: linear-gradient(
                        135deg,
                        color-mix(in oklab, var(--color-matte-gold) 92%, white 8%) 0%,
                        color-mix(in oklab, var(--color-matte-gold) 82%, black 18%) 100%
                      );
                      border-color: color-mix(in oklab, var(--color-matte-gold) 55%, transparent);
                      box-shadow: 0 16px 40px rgba(0, 0, 0, 0.52);
                    }

                    .bm-wa-fallback:hover .bm-wa-fallback__icon {
                      border-color: color-mix(in oklab, var(--color-matte-gold) 45%, transparent);
                      background: rgba(0, 0, 0, 0.22);
                    }

                    @keyframes bmWaPulse {
                      0%, 100% {
                        box-shadow: 0 12px 30px rgba(0, 0, 0, 0.35), 0 0 0 0 color-mix(in oklab, var(--color-matte-gold) 26%, transparent);
                      }
                      50% {
                        box-shadow: 0 12px 30px rgba(0, 0, 0, 0.35), 0 0 0 10px transparent;
                      }
                    }
                  `}</style>
                </div>
